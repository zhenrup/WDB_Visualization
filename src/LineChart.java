...
public class LineChart extends Viewers {
	 ...
	 */
	public LineChart(String title, Data[] datasets, int startYear, int endYear, String xLabel, String[] yLabel, String[][] seriesNames) {
		this.title = title;
	...
	public Viewers buildGraph() {
		// if analysis type returns only 1 value (i.e. an average) or when only 1 year is selected for a 1-series graph
			...
			for (int i = 0; i < datasets.length; i++) { // i represents the index of the Data object in datasets
				...
				plot.setRenderer(i, splinerenderer); // sets renderer	

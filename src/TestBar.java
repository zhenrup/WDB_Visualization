...
/**
 * Class TestBar creates and renders BarChart objects for all the analysis types it is compatible with (A1, A2, A3, A6, A7, A8)
...
public class TestBar {
	...
	public static void main(String[] args) {
		...
		BarChart sOne = new BarChart(aOne.getInstTitle(), aOne.calculate(), startYear, endYear, "Years", aOne.getUnits(), a1Series);
		pane.add(new JScrollPane(sOne.buildGraph()));
		
		BarChart sTwo = new BarChart(aTwo.getInstTitle(), aTwo.calculate(), startYear, endYear, "Years", aTwo.getUnits(), a2Series);
		pane.add(new JScrollPane(sTwo.buildGraph()));
		
		BarChart sThree = new BarChart(aThree.getInstTitle(), aThree.calculate(), startYear, endYear, "Years", aThree.getUnits(), a3Series);
		pane.add(new JScrollPane(sThree.buildGraph()));
		
		BarChart sSix = new BarChart(aSix.getInstTitle(), aSix.calculate(), startYear, endYear, "Years", aSix.getUnits(), a6Series);
		pane.add(new JScrollPane(sSix.buildGraph()));
		
		BarChart sSeven = new BarChart(aSeven.getInstTitle(), aSeven.calculate(), startYear, endYear, "Years", aSeven.getUnits(), a7Series);
		pane.add(new JScrollPane(sSeven.buildGraph()));
		
		BarChart sEight = new BarChart(aEight.getInstTitle(), aEight.calculate(), startYear, endYear, "Years", aEight.getUnits(), a8Series);
